...
$credential_name = trim($input['name'] ?? '');
if (empty($credential_name)) {
    $credential_name = 'Sicherheitsschlüssel';
}
...
try {
    ...
    $attestationObject = $attestationObjectLoader->load($attestationObject_b64);
    
    $authData = $attestationObject->authData;
    $credentialData = $authData->attestedCredentialData;
    
    if (!$credentialData) {
        throw new Exception("No credential data found in attestation");
    }

    $extracted_credential_id = $credentialData->credentialId;
    if ($extracted_credential_id !== $credential_id_raw) {
        throw new Exception("Credential ID mismatch");
    }

    $public_key_raw = $credentialData->credentialPublicKey; // Binary CBOR
    
    $decoder = new \CBOR\Decoder(new \CBOR\Tag\TagManager(), new \CBOR\OtherObject\OtherObjectManager());
    $coseKeyData = $decoder->decode(new \CBOR\StringStream($public_key_raw))->normalize();
